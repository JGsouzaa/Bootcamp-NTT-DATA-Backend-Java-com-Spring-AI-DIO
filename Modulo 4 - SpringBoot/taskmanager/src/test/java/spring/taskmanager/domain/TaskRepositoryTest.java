package spring.taskmanager.domain;
...
public abstract class TaskRepositoryTest {
    ...
    void should_save_and_retrieve_task_by_id(){
        //given
        var task = new Task("Go to bakery", Optional.empty());
    ...
    void should_find_all_persisted_tasks(){
        //given
        var task1 = new Task("Fix shower", Optional.of("Buy new shower"));
        var task2 = new Task("Change switch", Optional.of("Find screwdriver"));
    ...
    void should_delete_task_by_id(){
        //given
        var task = repository.save(new Task("Go to the gym", Optional.empty()));
    ...
    void should_update_task_status_successfully(){
        //given
        var task = repository.save(new Task("Go to market", Optional.empty()));

        task.setDescription(Optional.of("Don't need to buy soda"));
